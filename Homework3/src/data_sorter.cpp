...
auto DataSorter::SortSingleFile(const std::filesystem::directory_entry &file) -> bool {
  std::fstream out_file_stream(file.path());
  if (!out_file_stream) {
  ...
  // std::ofstream out_file_stream(file.path());
  out_file_stream.clear();
  out_file_stream.seekp(0, std::fstream::beg);
  for (const auto &data : buffer) {
    out_file_stream << data << '\n';
  }
  if (!out_file_stream) {
...
auto DataSorter::MergeTwoFiles(const std::string &file_path_1, const std::string &file_path_2) -> bool {
  ...
  std::ofstream output_file_stream(file_path_1 + "_merged");
  if (!output_file_stream) {
    std::cerr << "Couldn't create file: " << file_path_1 + "_merged" << '\n';
    return false;
  }

  // Merge the two files.
  std::int64_t data_1 = 0;
  std::int64_t data_2 = 0;
...
auto DataSorter::DoSort() -> bool {
  ...
  for (const auto &file : dir_it) {
    ...
    std::future<bool> res = thread_pool_.SubmitTask([&file] { return SortSingleFile(file); });
  ...
  do {
    ...
    while (file_it != files_to_merge.end()) {
      ...
      std::future<bool> res =
          thread_pool_.SubmitTask([file_it, next_file_it] { return MergeTwoFiles(*file_it, *next_file_it); });
